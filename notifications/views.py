...
def live_unread_notification_count(request):
    ...
    if not user_is_authenticated:
        data = {"unread_count": 0, "badge_html": ""}
    else:
        unread_count = request.user.notifications.unread().count()
        # Generate badge HTML if there are unread notifications
        badge_html = ""
        if unread_count > 0:
            badge_html = f'<span class="live_notify_badge" style="background-color: #dc3545; color: white; border-radius: 50%; padding: 2px 6px; font-size: 12px; font-weight: bold; display: inline-block; min-width: 18px; text-align: center;">{unread_count}</span>'
...
def live_unread_notification_list(request):
    ...
    if not user_is_authenticated:
        data = {"unread_count": 0, "unread_list": [], "badge_html": ""}
        return JsonResponse(data)
    ...
    if unread_count > 0:
        ...
        badge_html = f'<span class="{badge_class}">{unread_count}</span>'
