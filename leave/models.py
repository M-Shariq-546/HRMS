...
class LeaveType(HorillaModel):
    ...
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT, related_name='leave_type_company'
    )
    ...
    def get_avatar(self):
        ...
        if self.icon:
            ...
            file_url = file_service.GetImageUrl(self.icon)
            return file_url
        else:
    ...
    def set_expired_date(self, assigned_date):
        period = self.carryforward_expire_in if self.carryforward_expire_in is not None else 0
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(HorillaModel):
        leave_type_id = models.ForeignKey(
            LeaveType, on_delete=models.PROTECT, verbose_name="Leave type", null=True, blank=True
        )
        employee_id = models.ForeignKey(
            Employee, on_delete=models.CASCADE, verbose_name="Employee"
        )
        ...
        def save(self, *args, **kwargs):
            ...
            if not self.leave_type_id:
                leave_type = LeaveType.objects.create(
                    ...
                    payment='paid',
                    total_days=0,
                    company_id=self.employee_id.employee_work_info.company_id,
                    is_compensatory_leave=True,
                )
                self.leave_type_id = leave_type
            super().save(*args, **kwargs)
