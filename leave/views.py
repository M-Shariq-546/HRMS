...
from .leaves_time_method import compensation_leaves_notification_department_managers_or_admin,compensation_leaves_notification_request_user
...
def leave_type_creation(request):
    ...
    if request.method == "POST":
        form = LeaveTypeForm(request.POST, request.FILES, request=request)
        if form.is_valid():
            leave_type = form.save()
            file_url = FileStorageService.UploadImage(leave_type.icon, image_type='leave')
            leave_type.icon = file_url
            leave_type.save()
...
def leave_type_view(request):
    ...
    queryset = None
    if company_id != 'all':
        queryset = LeaveType.objects.filter(company_id_id=company_id).exclude(is_compensatory_leave=True)
    else:
    ...
    if (
        ...
        if company_id != 'all':
            queryset = LeaveType.objects.filter(company_id_id=company_id)
        else:   
...
def leave_type_update(request, id, **kwargs):
    ...
    if request.method == "POST":
        request_icon = request.FILES.getlist("icon")
        if request_icon:
            previous_icon = leave_type.icon
        form_data = UpdateLeaveTypeForm(
            request.POST, request.FILES, instance=leave_type
        )
        if form_data.is_valid():
            updated_instance = form_data.save()
            if request_icon:
                file_url = FileStorageService.UploadImage(updated_instance.icon, image_type='leave')
                updated_instance.icon = file_url
                updated_instance.save()
                FileStorageService.DeleteImage(previous_icon)
...
def leave_type_delete(request, obj_id):
    ...
    try:
        ...
        FileStorageService.DeleteImage(leave_type.icon)
        leave_type.delete()
...
def leave_request_update(request, id):
    ...
    if request.method == "POST":
        ...
        if attachment:
            previous_attachment = leave_request.attachment
        ...
        if form.is_valid():
            ...
            save = True
            if attachment:
                file_url = FileStorageService.UploadImage(leave_request.attachment, image_type='leave')
...
def leave_request_delete(request, id):
    ...
    try:
        ...
        file_service.DeleteImage(leave_request.attachment)
...
def leave_request_approve(request, id, emp_id=None):
    ...
    logger.info(f'company name checking {leave_request.leave_type_id.company_id.company}')
...
def user_request_update(request, id):
    ...
    try:
        if (
            ...
            if request.method == "POST":
                request_attachment = request.FILES.getlist("attachment")
...
def leave_allocation_request_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            leave_allocation_request = form.save()
            file_url = FileStorageService.UploadImage(leave_allocation_request.attachment, image_type='leave_allocation_request')
            leave_allocation_request.attachment = file_url
...
def leave_allocation_request_update(request, req_id):
    ...
    if leave_allocation_request.status != "approved":
        if request.method == "POST":
            attachment = request.FILES.getlist("attachment")
            if attachment:
                previous_attachment = leave_allocation_request.attachment
...
def leave_allocation_request_delete(request, req_id):
    ...
    try:
        ...
        if leave_allocation_request.status != "approved":
            FileStorageService.DeleteImage(leave_allocation_request.attachment)
...
if apps.is_installed("attendance"):
    ...
    def create_compensatory_leave(request, comp_id=None):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                comp_req = form.save(commit=False)
                days = attendance_days(
                    comp_req.employee_id, form.cleaned_data["attendance_id"]
                )
                if not days:
                    form.add_error(
                        None,
                        _("Selected attendance record(s) do not yield compensable hours."),
                    )
                    return render(request, template, {"employee": employee, "form": form})
                
                comp_req.requested_days = days
                comp_req.save()
                form.save_m2m()
                if comp_id is not None:
                    messages.success(request, _("Compensatory Leave updated."))
                else:
                    compensation_leaves_notification_request_user(request.user, 'created')
                    compensation_leaves_notification_department_managers_or_admin(employee)
    ...
    def approve_compensatory_leave(request, comp_id):
        ...
        try:
            ...
            if comp_leave_req.status == "requested":
                if (
                    request.user.employee_get
                    == comp_leave_req.created_by.employee_get.employee_work_info.reporting_manager_id
                ):
                    comp_leave_req.status = 'reviewed'
                    comp_leave_req.save()
                    messages.success(request, _("Compensatory leave request reviewed by manager and forwarded for approval."))
                    with contextlib.suppress(Exception):
                        notify.send(
                            request.user.employee_get,
                            recipient=comp_leave_req.employee_id.employee_user_id,
                            verb=_("Your compensatory leave request has been reviewed and forwarded for approval"),
                            verb_ar = "تمت مراجعة طلب الإجازة التعويضية الخاص بك وتمت إحالته للموافقة",
                            verb_de = "Ihr Antrag auf Freizeitausgleich wurde überprüft und zur Genehmigung weitergeleitet",
                            verb_es = "Su solicitud de permiso compensatorio ha sido revisada y enviada para su aprobación",
                            verb_fr = "Votre demande de congé compensatoire a été examinée et transmise pour approbation",
                            icon="people-circle",
                        ...
                        hr_users = User.objects.filter(
                            ...
                            Q(user_permissions__codename__in=[
                                'add_compensatoryleaverequest',
                                'view_compensatoryleaverequest',
                                'change_compensatoryleaverequest',
                                'delete_compensatoryleaverequest',
                            ]) |
                            Q(groups__permissions__codename__in=[
                                'add_compensatoryleaverequest',
                                'view_compensatoryleaverequest',
                                'change_compensatoryleaverequest',
                                'delete_compensatoryleaverequest',
                        ...
                        notify.send(
                            request.user.employee_get,
                            recipient=list(hr_users),
                            verb=_("A compensatory leave request is awaiting your approval."),
                            icon="people-circle",
            ...
            elif comp_leave_req.status == "reviewed" and request.user.has_perm('leave.delete_compensatoryleaverequest'):
                comp_leave_req.status = "approved"
                comp_leave_req.assign_compensatory_leave_type()
                comp_leave_req.save()
                messages.success(request, _("Compensatory leave request approved."))
                ...
                messages.info(
                    request,
                    _(
                        "The compensatory leave request is not in the 'valid' status."
                    ),
                )
        except Exception as e:
