...
from .leaves_time_method import str_to_seconds
...
def get_leave_day_attendance(employee, comp_id=None):
    ...
    attendances_to_exclude = []
    for attendance in attendances:
        worked_hours = str_to_seconds(attendance.attendance_worked_hour)
        minimum_hours = str_to_seconds(attendance.minimum_hour)
        attendance_time = worked_hours - minimum_hours
        if attendance_time < 10800:
            attendances_to_exclude.append(attendance)
    comp_leave_reqs = None
    if (
        ...
        attendances_to_exclude = set(attendances_to_exclude)
        for req in comp_leave_reqs:
            attendances_to_exclude |= set(req.attendance_id.all())
    # Filter holiday attendance excluding the attendances in attendances_to_exclude
    attendances_to_exclude_ids = {attendance.id for attendance in attendances_to_exclude}
    holiday_attendance = Attendance.objects.filter(
        is_holiday=True, employee_id=employee, attendance_validated=True
    ).exclude(id__in=attendances_to_exclude_ids)
