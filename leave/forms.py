...
from base.models import CompanyLeaves, Holidays, Company
...
logger = HorillaLogger('horilla.leave.forms')
...
class LeaveTypeForm(ConditionForm):
    ...
    def save(self, *args, **kwargs):
        ...
        if selected_company and selected_company != 'all':
            leave_type.company_id = Company.objects.get(id=selected_company)
            leave_type.save()
    ...
    def __init__(self, *args, **kwargs):
        ...
        selected_company = self.request.session.get('selected_company')
        logger.info(f'selected_company is {selected_company}')
        if selected_company and selected_company != 'all':
            self.fields['employee_id'].queryset = Employee.objects.filter(employee_work_info__company_id=selected_company).distinct()
            logger.info(f'employee_id queryset is {self.fields["employee_id"].queryset}')
        else:
            self.fields['employee_id'].queryset = Employee.objects.none()
...
class LeaveRequestUpdationForm(ModelForm):
    ...
    def save(self, commit=True):
        ...
        instance.attachment = self.cleaned_data.get("attachment")
        instance.save()
        return instance
    ...
    class CompensatoryLeaveForm(ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if instance_id is not None:
                self.fields["attendance_id"].choices = self.instance.attendance_id.all().values_list("id", "attendance_date")
