...
from payroll.forms.synetec_forms import CompanyTimezoneForm
import pandas as pd
...
logger = HorillaLogger("horilla.views")
...
class HorillaPasswordResetView(PasswordResetView):
    ...
    html_email_template_name = 'reset-password.html'
    ...
    def form_valid(self, form):
        ...
        if user:
            opts = {
                ...
                "company":user.employee_get.employee_work_info.company_id.company,
...
class EmployeePasswordResetView(PasswordResetView):
    ...
    def form_valid(self, form):
        logger.info("EmployeePasswordResetView form_valid called")
        try:
            email_backend = ConfiguredEmailBackend()

            username = form.cleaned_data["email"]
            user = User.objects.filter(username=username).first()
            logger.info(f"user company : {user.employee_get.employee_work_info.company_id.icon}")
            if user:
                opts = {
                    "use_https": self.request.is_secure(),
                    "token_generator": self.token_generator,
...
def custom_dispatch(self, request, *args, **kwargs):
    logger.info('=== Starting custom_dispatch ===')
    ...
    try:
        ...
        logger.info(f'Error getting user: {str(e)}')
        user = None
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        logger.info('Token is invalid or user is None')
        self.validlink = False
        messages.error(request, _("This password reset link has expired or has already been used."))
        return redirect("home-page")
    
    logger.info('=== Ending custom_dispatch ===')
...
setattr(PasswordResetConfirmView, "dispatch", custom_dispatch)
...
def mail_server_test_email(request):
    ...
    image_path = os.path.join(settings.BASE_DIR, "static", "images", "horilla.png")
...
def company_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            company = form.save()
            if company.icon:
                file_url = FileStorageService.UploadImage(company.icon, image_type='company')
                company.icon = file_url
                company.save()
...
def company_update(request, id, **kwargs):
    ...
    if request.method == "POST":
        icon_change = request.FILES.getlist("icon")
        if icon_change:
            previous_icon = company.icon
...
def clear_notification(request):
    """
    This method is used to clear notification and mark as read
    """
    try:
...
def general_settings(request):
    ...
    timezone_form = CompanyTimezoneForm(initial=initial_data)
...
def currency_symbol():
    ...
    currency = symbol.currency_code if symbol else "$"
...
def get_notification_badge(request):
    """
    This method returns the notification badge HTML for dynamic updates
    """
    from notifications.templatetags.notifications_tags import live_notify_badge
    from django.template import Context
    from django.http import HttpResponse
    
    badge_html = live_notify_badge(Context({'request': request, 'user': request.user}))
...
def employees_active_contract_list(request):
    name= request.GET.get('q')
    employees_ids = request.GET.getlist('exclude[]')
    company_id = request.session.get('selected_company')
    exclude_ids = [int(eid) for eid in employees_ids if eid.isdigit()]
    employees = Employee.objects.filter(contract_set__contract_status='active')
    
    if company_id != 'all':
        employees = employees.filter(employee_work_info__company_id=company_id)
        
    if employees_ids:
        employees = employees.exclude(id__in=exclude_ids)
    
    if name:
        employees= employees.filter(Q(employee_first_name__icontains=name)| Q(employee_last_name__icontains=name))
    
    results = [{'id': emp.id, 'name': emp.get_full_name()} for emp in employees]
