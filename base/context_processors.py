...
def get_companies(request):
    ...
    """
    companies = list(
        [company.id, company.company, company.icon.url, False, company.get_icon_url()]
    ...
    if company_id != "all":
        if re.match(pattern, previous_path):
            ...
            if (
                ...
                text = "Other Company"
                if (
                    company_id == request.user.employee_get.employee_work_info.company_id
                ):
                    text = "My Company"
                if company_id == "all":
                    text = "All companies"
                    ...
                    company = {
                        "company": company.company,
                        "icon": company.icon.url,
                        "get_icon_url": company.get_icon_url(),
                        "text": text,
                        "id": company.id,
                    }
                messages.error(
...
def white_labelling_company(request):
    ...
    if white_labelling:
        ...
        return {
            "white_label_company_name": company.company if company else os.environ.get("PRODUCT_NAME", 'HRMS'),
            "white_label_company": company,
        }
    else:
        return {
            "white_label_company_name": os.environ.get("PRODUCT_NAME", 'HRMS'),
