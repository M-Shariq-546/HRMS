...
# your form here
logger = HorillaLogger("base.forms")
...
class CompanyForm(ModelForm):
    ...
    class Meta:
        ...
        model = Company
        # fields = "__all__"
        exclude = ["date_format", "time_format", "is_active", "timezone"]
...
class MailTemplateForm(ModelForm):
    ...
    VARIABLE_MAPPING = {
        "{{ Receiver_Full_name }}": "instance.get_full_name()",
        "{{ Sender_Full_name }}": "self.get_full_name()",
        "{{ Receiver_Recruitment }}": "instance.recruitment_id",
        "{{ Sender_Recruitment }}": "self.recruitment_id",
        "{{ Receiver_Company }}": "instance.get_company()",
        "{{ Sender_Company }}": "self.get_company()",
        "{{ Receiver_Job_position }}": "instance.get_job_position()",
        "{{ Sender_Job_position }}": "self.get_job_position()",
        "{{ Receiver_Email }}": "instance.get_email()",
        "{{ Sender_Email }}": "self.get_email()",
        "{{ Candidate_Full_name }}": "instance.get_full_name()",
        "{{ Candidate_Recruitment }}": "instance.recruitment_id",
        "{{ Candidate_Company }}": "instance.get_company",
        "{{ Candidate_Job_position }}": "instance.get_job_position",
        "{{ Candidate_Email }}": "instance.get_email",
        "{{ Candidate_Interview_Table }}": "instance.get_interview",
    }

    @classmethod
    def render_template(cls, template_body, context):
        """
        Replace placeholders in the template with actual values
        """
        from django.template import Template, Context
        if isinstance(template_body, str):
            template_body = Template(template_body)  # Convert string to Django Template

        rendered_body = template_body.render(Context(context))  # Process template
        for placeholder, value in cls.VARIABLE_MAPPING.items():
            if placeholder in rendered_body:
                try:
                    actual_value = eval(value, {}, context)  # Safely evaluate the expression
                    rendered_body = rendered_body.replace(placeholder, str(actual_value))
                except Exception as e:
                    rendered_body = rendered_body.replace(placeholder, "[ERROR]")
        return rendered_body

    @classmethod
    def render_subject(cls, subject, context):
        """
        Replace placeholders in the template with actual values
        """
        rendered_subject =  subject # Process template
        for placeholder, value in cls.VARIABLE_MAPPING.items():
            if placeholder in rendered_subject:
                try:
                    actual_value = eval(value, {}, context)  # Safely evaluate the expression
                    rendered_subject = rendered_subject.replace(placeholder, str(actual_value))
                except Exception as e:
                    rendered_subject = rendered_subject.replace(placeholder, "[ERROR]")
...
class PassWordResetForm(forms.Form):
    ...
    def send_mail(
        ...
        context['white_label_company_name'] = white_labelling_company(request)['white_label_company_name']
        ...
        # Send email using Synetech service
        ...
        try:
            ...
            logger.info(f"Error sending email via Synetech API: {str(e)}")
        if not success:
            raise Exception("Failed to send email via Synetech API")
    ...
    def save(
        ...
        logger.info(f"company inside save: {company}")
        if email:
