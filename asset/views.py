...
logger = HorillaLogger('asset.views')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                assigned_images = []
                for file in files:
                    attachment = ReturnImages.objects.create(image=file)
                    assigned_images.append(attachment)

                allocation.assign_images.add(*assigned_images)
                allocation.save()
                
                asset_request.asset_request_status = "Approved"
                asset_request.save()

                for allocation_image in allocation.assign_images.all():
                    file_url = FileStorageService.UploadImage(allocation_image.image, image_type='assets')
                    allocation_image.image = file_url
                    allocation_image.save()
...
def asset_allocate_creation(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for image in instance.assign_images.all():
                file_url = FileStorageService.UploadImage(image.image, image_type='assets')
    ...
    if request.method == "POST":
        ...
        if asset_return_form.is_valid():
            ...
            if asset_return_status == "Healthy":
                ...
                for image in asset_allocation.return_images.all():
                    file_url = FileStorageService.UploadImage(image.image, image_type='assets')
                    image.image = file_url
                    image.save()
...
def asset_tab(request, emp_id):
    ...
    assets = employee.allocated_employee.filter(return_status=None).distinct()
    print('======== assets', assets.values())
    assets_ids = (
...
def profile_asset_tab(request, emp_id):
    ...
    """
    employee = Employee.objects.get(id=emp_id)
    assets = employee.allocated_employee.filter(asset_id__asset_status="In use")
...
@login_required
@hx_request_required
@permission_required(perm="asset.change_assetrequest")
def asset_request_update(request, asset_request_id):
    """
    This function is used to update the asset request.
    Args:
        request (HttpRequest): The HTTP request object
        asset_request_id (int): The ID of the asset request to update
    Returns:
        HttpResponse: Rendered template with the form for updating the asset request
    """
    try:
        asset_request = get_object_or_404(AssetRequest, id=asset_request_id)
        form = AssetRequestForm(user=request.user, instance=asset_request)
        ...
        if request.method == "POST":
            ...
            if not post_data.get('requested_employee_id'):
                post_data['requested_employee_id'] = asset_request.requested_employee_id.id
            if not post_data.get('asset_category_id'):
                post_data['asset_category_id'] = asset_request.asset_category_id.id
                
            form = AssetRequestForm(post_data, user=request.user, instance=asset_request)
            if form.is_valid():
                form.save()
                messages.success(request, _("Asset request updated successfully"))
                return HttpResponse("<script>window.location.reload();</script>")
            else:
                messages.error(request, _("Please correct the errors below"))
        
        context = {
            "asset_request_form": form,
            "asset_request": asset_request,
            "is_update": True
        }
        return render(request, "request_allocation/asset_request_update.html", context=context)
    except Exception as e:
        messages.error(request, _("An error occurred while updating the asset request"))
        return HttpResponse("<script>window.location.reload();</script>")
