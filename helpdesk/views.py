...
def faq_category_delete(request, id):
    '''
    This FAQ category cannot be deleted if there is any faq in this category because faq category has foreign-key relation with faq and is 
    handeled as protected in on_delete case
    '''
    try:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if ticket.frequency == 'custom':
                ...
                if cron_expression:
                    ticket.cron_expression = cron_expression
                    ticket.save()
            ...
            for image in ticket.ticket_attachment.all():
                file_url = FileStorageService.UploadImage(image.file, image_type='helpdesk')
                image.file = file_url
                image.save()
            
            department, job_position, user, department_managers = Department_job_user_query(request.POST)
            mail_thread = TicketSendThread(request, ticket, department_managers, type="create")
            mail_thread.start()
            ...
            notification_to_request_user(request.user, assignees, ticket)
            if request.POST.get('assigning_type') == 'department' and not request.user.is_superuser:
                notification_to_managers_users(request.user, department_managers, ticket, 'Department', department)
            elif request.POST.get('assigning_type') == 'job_position' and not request.user.is_superuser:
                notification_to_managers_users(request.user, department_managers, ticket, 'Job Role', job_position)
            elif request.POST.get('assigning_type') == 'individual':
                notification_to_individual_user(request.user, assignees, ticket, 'Individual', user)

            return HttpResponse("<script>window.location.reload()</script>")
    context = {
        "form": form,
        "t_type_form": TicketTypeForm(initial={'company_id': company_id}),
    }
            ...
            if form.is_valid():
                ...
                if ticket.frequency == 'custom':
                    ...
                    if cron_expression:
                        ...
                        # Reschedule the job if cron expression or frequency changed to custom
                        if cron_expression and ticket.frequency == 'custom':
...
def change_ticket_status(request, ticket_id):
    ...
    ticket = Ticket.objects.get(id=ticket_id)
    response = None
    pre_status = ticket.get_status_display_with_key(ticket.status)
    ...
    if ticket.status != status:
        if (
            ...
            or request.user.has_perm("helpdesk.change_ticket") or (ticket.assigning_type == 'department' and ticket.get_raised_on_object().dept_manager.filter(manager__employee_user_id=request.user).exists()) or (ticket.assigning_type == 'job_position' and ticket.get_raised_on_object().department_id.dept_manager.filter(manager__employee_user_id=request.user))
        ):
            ticket.status = status
            ticket.save()
            current_status = ticket.get_status_display_with_key(status)
            time = datetime.now()
            ...
            if request.user == ticket.created_by:
                managers = None
                if ticket.assigning_type == 'department':
                    managers = ticket.get_raised_on_object().dept_manager.all()
                elif ticket.assigning_type == 'job_position':
                    managers = ticket.get_raised_on_object().department_id.dept_manager.all()
                elif ticket.assigning_type == 'individual':
                    managers = Employee.objects.get(id=ticket.raised_on)
                ...
                notify.send(
                    request.user.employee_get,
                    recipient=assignees if request.user == ticket.created_by else ticket.created_by,
                    verb=f"The status of the ticket has been changed to {ticket.status}.",
                    verb_ar="تم تغيير حالة التذكرة.",
                    verb_de="Der Status des Tickets wurde geändert.",
                    verb_es="El estado del ticket ha sido cambiado.",
            ...
            status_priority_update_history(ticket, previous_status=pre_status, new_status=current_status, employee=request.user.employee_get)
            managers = None
            if ticket.assigning_type == 'department':
                managers = ticket.get_raised_on_object().dept_manager.all()
            else:
    ...
    if response is None:
        response = {
            "type": "info",
            "message": _("Ticket status was not changed because it's already set."),
        }
    return JsonResponse(response)
    ...
    try:
        ...
        if ticket.status == "new":
            mail_thread = TicketSendThread(
                request,
                ticket,
                ticket.get_raised_on_object().dept_manager.all() if hasattr(ticket.get_raised_on_object(), "dept_manager") else [],
                type="delete",
            )
            mail_thread.start()
            employees = ticket.assigned_to.all()
            ...
            for image in ticket.ticket_attachment.all():
                FileStorageService.DeleteImage(image.file)
            ticket.delete()
            messages.success(
                request,
...
def ticket_detail(request, ticket_id, **kwargs):
    ...
    if (
        ...
        trackings = ticket.get_all_history()
        ...
        for history in trackings:
            history_data = {
                ...
                "history": {
                    ...
                    "changes": [{
                        "field": history.previous, 
                        "old": history.previous,
                        "new": history.new,
                        "is_fk": False, 
                    }],
                    "pair": [history],
            ...
            activity_list.append(history_data)
        ...
        messages.info(request, _("You don't have permission."))
        redirect_url = reverse("ticket-view")
...
def ticket_change_raised_on(request, ticket_id):
    ...
    if (
        request.user.has_perm("helpdesk.view_ticket")
        or request.user.employee_get == ticket.employee_id or (ticket.assigning_type == 'department' and ticket.get_raised_on_object().dept_manager.filter(manager__employee_user_id=request.user).exists()) or (ticket.assigning_type == 'job_position' and ticket.get_raised_on_object().department_id.dept_manager.filter(manager__employee_user_id=request.user).exists()) 
    ):
        form = TicketRaisedOnForm(instance=ticket)
        previous_assigning_type = ticket.assigning_type
        previous_raised_on = ticket.raised_on
        if request.method == "POST":
            try:
                form = TicketRaisedOnForm(request.POST, instance=ticket)
                if form.is_valid():
                    form.save()
                    new_assigning_type = ticket.assigning_type
                    new_raised_on = ticket.raised_on
                    ticket.get_ticket_history(previous_raised_on, new_raised_on, request.user, previous_assigning_type, new_assigning_type)
                    messages.success(request, _("Responsibility updated for the Ticket"))
                    department, job_position, user, department_managers = Department_job_user_query(request.POST)
                    if request.POST.get('assigning_type') == 'department':
                        notification_to_managers_users(request.user, department_managers, ticket, 'department', department) 
                    elif request.POST.get('assigning_type') == 'job_position':
                        notification_to_managers_users(request.user, department_managers, ticket, 'job_position', job_position) 
                    else:
                        notification_to_individual_user(request.user, None, ticket, 'Individual', user)

                    if request.headers.get('HX-Request'):
                        response = HttpResponse()
                        response["HX-Redirect"] = reverse("ticket-view")
                        return response
                    else:
                        return redirect("ticket-view")
            except Exception as e:
                print('Exception in change raised on', e)
        return render(
            request,
    ...
    if request.user.has_perm("helpdesk.change_ticket") or is_department_manager(
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                send_assigning_ticket_notification(request, added_assignees, ticket)
                send_removing_ticket_notification(request, removed_assignees, ticket)
                return HttpResponse("<script>window.location.reload()</script>")
...
def view_ticket_document(request, doc_id):
    ...
    if document_obj.file:
        ...
        file_url = file_service.GetImageUrl(document_obj.file)
        context["file_url"] = file_url
    elif document_obj.file:
...
def delete_ticket_document(request, doc_id):
    ...
    response = HttpResponse()
    response["HX-Redirect"] = reverse("ticket-detail", args=[ticket_id])
    return response
...
def comment_create(request, ticket_id):
    ...
    if not (
        ...
        messages.info(request, _("You don't have permission to comment on this ticket."))
        return redirect("ticket-view")
    if request.method == "POST":
        ticket = Ticket.objects.get(id=ticket_id)
        ...
        if c_form.is_valid():
            ...
            comment_sent_notification(request.user, ticket.created_by, ticket.assigned_to.all(), ticket)
...
def claim_ticket(request, id):
    ...
    if not ClaimRequest.objects.filter(
        ...
        messages.success(request, 'Your Claim Request has been sent successfully')
        horilla_bot = User.objects.get(username='Horilla Bot')
        claim_request_notification_to_self_user(horilla_bot, request.user)
        claim_request_to_managers(request.user, ticket)
    else:
        messages.error(request, 'You already requested for this ticket')
    return HttpResponseRedirect(request.META.get("HTTP_REFERER", "/"))
...
def ticket_type_create(request):
    ...
    if company_id == 'all':
        messages.error(request, _("Please select a company first."))
        return HttpResponse("<script>window.location.reload()</script>")
    form = TicketTypeForm(initial={'company_id': company_id})
    if request.method == "POST":
