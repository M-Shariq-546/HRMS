...
from helpdesk.utils import HOUR_CHOICES, MINUTE_CHOICES
...
class TicketTypeForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'initial' in kwargs and 'company_id' in kwargs['initial']:
            company_id = kwargs['initial']['company_id']
            self.fields['company_id'].queryset = self.fields['company_id'].queryset.filter(id=company_id)
    ...
    def clean(self):
        ...
        if frequency == 'custom':
            if not cron_expression:
                raise forms.ValidationError("Please enter a CRON expression")
            # Basic CRON format validation
            parts = cron_expression.split()
            if len(parts) != 5:
                raise forms.ValidationError("Invalid CRON expression format. Expected format: minute hour day month weekday")
