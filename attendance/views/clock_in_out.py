...
def get_random_message_from_file(filename):
    full_path = os.path.join(settings.BASE_DIR, 'static', filename)
...
def clock_in(request):
    ...
    if (
        ...
        employee_timezone = pytz.timezone(employee.timezone if employee.timezone else "UTC")

        # Determine datetime_now with timezone-awareness
        datetime_now = datetime.now(employee_timezone)  # Assume UTC first
        if request.__dict__.get("datetime"):
            datetime_now = request.datetime

        # Ensure it's timezone-aware
        if datetime_now.tzinfo is None or datetime_now.tzinfo.utcoffset(datetime_now) is None:
            datetime_now = make_aware(datetime_now)

        # Convert datetime to employee timezone
        datetime_now = datetime_now.astimezone(employee_timezone)
        if employee and work_info is not None:
            ...
            logger.info(f"{employee.employee_first_name} {employee.employee_last_name} just Checked-In")
            if company_integration:
                slack_service = SlackService.SendMessage(company_integration, employee, datetime_now, get_random_message_from_file('checkin.txt'))
...
def clock_out(request):
    ...
    if (
        ...
        # Determine datetime_now with timezone-awareness
        datetime_now = datetime.now(pytz.utc)  # Assume UTC first
        if request.__dict__.get("datetime"):
            datetime_now = request.datetime
        ...
        logger.info(f"{employee.employee_first_name} {employee.employee_last_name} Checked out at {datetime_now} at {datetime_now.time()}")
        if company_integration:
            slack_service = SlackService.SendMessage(company_integration, employee, datetime_now, get_random_message_from_file('checkout.txt'))
        if attendance:
            early_out_instance = attendance.late_come_early_out.filter(type="early_out")
