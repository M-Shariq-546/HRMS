...
logger = HorillaLogger('attendance.models')
...
class Attendance(HorillaModel):
    ...
    def serialize(self):
        ...
        serialized_data = {
            ...
            "request_description":str(self.request_description) if self.request_description else "",
    ...
    def clean(self, *args, **kwargs):
        ...
        if self.attendance_clock_in_date > self.attendance_date:
            raise ValidationError(
                {
                    "attendance_clock_in_date": "Attendance check-in date never be in future date of attendance date"
                }
            )
        if (
        ...
        if (
            ...
            raise ValidationError(
                {
                    "attendance_clock_out_date": "Attendance check-out date never be in future date of attendance check-in date"
                }
            )
...
"""
This is the Signal for Employee to update the time of checkin and checkout based on selected timezone
"""
@receiver(pre_save, sender=Employee)
def update_attendance_on_timezone_change(sender, instance, **kwargs):
    """
    Updates attendance records when an employee's timezone is changed.
    """
    import pytz
    ...
    if instance.pk:  # Ensure this is an update, not a new record
        try:
            ...
            if old_instance.timezone != instance.timezone:
                # Timezone has changed, update related attendance records
                attendances = Attendance.objects.filter(employee_id=instance)
                new_tz = pytz.timezone(instance.timezone)  # Get new timezone

                for attendance in attendances:
                    if attendance.attendance_clock_in:
                        # Make datetime aware if it's naive
                        if attendance.attendance_clock_in.tzinfo is None:
                            attendance_clock_in_dt = datetime.combine(today_date, attendance.attendance_clock_in)
                            attendance.attendance_clock_in = make_aware(attendance_clock_in_dt)
                            attendance_clock_in_dt = attendance_clock_in_dt.astimezone(new_tz)

                        attendance.attendance_clock_in = attendance_clock_in_dt.time()

                    if attendance.attendance_clock_out:
                        if attendance.attendance_clock_out.tzinfo is None:
                            attendance_clock_out_dt = datetime.combine(today_date, attendance.attendance_clock_out)
                            attendance.attendance_clock_out = make_aware(attendance_clock_out_dt)
                            attendance_clock_out_dt = attendance_clock_out_dt.astimezone(new_tz)

                        attendance.attendance_clock_out = attendance_clock_out_dt.time()
