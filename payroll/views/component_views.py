...
from horilla.methods import dynamic_attr, get_horilla_model_class, GetImageUrlencode
...
from payroll.methods.synetec_tax_calc import calculate_taxable_amount_pakistan
...
logger = HorillaLogger("payroll.component_views")
...
def payroll_calculation(employee, start_date, end_date):
    ...
    employee_information = EmployeeWorkInformation.objects.get(employee_id=employee)
    ...
    sumbol = PayrollSettings.objects.get(company_id=employee_information.company_id)
    if PayrollSettings.objects.filter(company_id=employee_information.company_id, currency_symbol='PKR (₨)').exists():
        federal_tax = calculate_taxable_amount_pakistan(**kwargs)
    else:
...
def view_allowance(request):
    ...
    if selected_company == 'all':
        ...
        allowances = Allowance.objects.filter(company_id=selected_company)
...
def view_single_allowance(request, allowance_id):
    ...
    """
    previous_data = GetImageUrlencode(request)
...
def create_deduction(request):
    ...
    if request.method == "POST":
        form = forms.DeductionForm(request.POST, request=request)
        if form.is_valid():
            form.save()
...
def view_deduction(request):
    ...
    if company == 'all':
        ...
        deductions = Deduction.objects.filter(company_id=company)
...
def view_payslip(request):
    ...
    filter_form = PayslipFilter(data=request.GET, queryset=payslips)
...
def filter_payslip(request):
    ...
    session_key = "payslip_query_string"

    if query_string:
        # Save current query to session
        request.session[session_key] = query_string
        logger.info("Stored query_string in session.")
...
def send_slip(request):
    ...
    try:
        ...
        if employee_id.get_mail() is None:
            messages.warning(request, _('Please set Employee Email Address'))
            return HttpResponse("<script>window.location.reload()</script>")

        # Create email thread for sending payslips
        ...
        logger.error(f"Error in send_slip functions of service : {e}")
        return HttpResponse("<script>window.location.reload()</script>")
...
def add_bonus(request):
    ...
    if payslip_id != "None" and payslip_id:
        ...
        form = forms.PayslipAllowanceForm(
            initial={"employee_id": employee_id, "date": instance.start_date, "company_id": company.id}
        )
    else:
        form = forms.BonusForm(initial={"employee_id": employee_id, "company_id": company.id})
    if request.method == "POST":
        form = forms.BonusForm(request.POST, initial={"employee_id": employee_id, "company_id": company.id})
        contract = Contract.objects.filter(
            employee_id=employee_id, contract_status="active"
        ).first()
...
def add_deduction(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            deduction_instance.company_id = company
...
def view_installments(request):
    ...
    previous_id = None
    next_id = None
    # Sort installments by one_time_date (chronological order)
    installments = loan.deduction_ids.all().order_by("one_time_date")
...
def edit_installment_amount(request):
    ...
    deductions = list(loan.deduction_ids.all().order_by("one_time_date"))
    deduction = next((d for d in deductions if str(d.id) == str(ded_id)), None)
    if not deduction:
        messages.error(request, "Installment not found.")
        return redirect(f"{reverse('view-installments')}?loan_id={loan_id}")

    # Mark paid installments (those with a payslip)
    paid_deductions = [d for d in deductions if d.installment_payslip()]
    unpaid_deductions = [d for d in deductions if not d.installment_payslip()]

    # Find index of edited installment in unpaid list
    unpaid_idx = next((i for i, d in enumerate(unpaid_deductions) if d.id == deduction.id), None)
    if unpaid_idx is None:
        messages.error(request, "Cannot edit a paid installment.")
        return redirect(f"{reverse('view-installments')}?loan_id={loan_id}")

    # If setting to 0 (skip month):
    if value == 0:
        skipped_amount = deduction.amount
        deduction.amount = 0
        deduction.save()
        # Find the latest one_time_date among all installments
        last_date = max([d.one_time_date for d in deductions])
        # Add a new month at the end with the skipped amount
        from base.methods import get_next_month_same_date
        new_date = get_next_month_same_date(last_date)
        new_installment = _create_deductions(loan, skipped_amount, new_date)
        loan.deduction_ids.add(new_installment)
        messages.success(request, "Installment skipped. New month added with skipped amount.")
        return redirect(f"{reverse('view-installments')}?loan_id={loan_id}")

    # If changing a previously skipped (0) month to non-zero:
    if deduction.amount == 0 and value > 0:
        deduction.amount = value
        deduction.save()
        # Recalculate the last month to ensure total matches loan amount
        all_deductions = list(loan.deduction_ids.all().order_by("one_time_date"))
        total_paid = sum(d.amount for d in all_deductions)
        diff = total_paid - loan.loan_amount
        if diff > 0:
            # Reduce the last month by the extra
            last_ded = all_deductions[-1]
            last_ded.amount = max(last_ded.amount - diff, 0)
            last_ded.save()
            # If last month is now 0, remove it
            if last_ded.amount == 0:
                loan.deduction_ids.remove(last_ded)
                last_ded.delete()
        elif diff < 0:
            # Add the missing amount to the last month
            last_ded = all_deductions[-1]
            last_ded.amount += abs(diff)
            last_ded.save()
        messages.success(request, "Installment updated. Last month adjusted to maintain total.")
        return redirect(f"{reverse('view-installments')}?loan_id={loan_id}")

    # For normal update (not skipping, not restoring):
    deduction.amount = value
    deduction.save()
    # Ensure the last month absorbs any rounding or leftover
    all_deductions = list(loan.deduction_ids.all().order_by("one_time_date"))
    total_paid = sum(d.amount for d in all_deductions)
    diff = total_paid - loan.loan_amount
    if diff != 0:
        ...
        if last_ded.amount == 0:
            loan.deduction_ids.remove(last_ded)
            last_ded.delete()
    messages.success(request, "Installment amount updated and total maintained.")
    return redirect(f"{reverse('view-installments')}?loan_id={loan_id}")
...
def create_reimbursement(request):
    ...
    if instance_id:
        previous_instance = Reimbursement.objects.filter(id=instance_id).first()
        instance = previous_instance
    form = forms.ReimbursementForm(instance=instance)
    if request.method == "POST":
        if instance:
            previous_attachment = instance.attachment
        form = forms.ReimbursementForm(request.POST, request.FILES, instance=instance)
        if form.is_valid():
            instance , attachemnts = form.save()
            for attachment in instance.other_attachments.all():
                file_url = FileStorageService.UploadImage(attachment.attachment, image_type='reimbursement')
                attachment.attachment = file_url
                attachment.save()
                if previous_attachment and previous_instance:
                    FileStorageService.DeleteImage(previous_attachment)
...
def delete_reimbursements(request):
    ...
    file_service.DeleteImage(reimbursement.attachment)
...
def delete_attachments(request, _reimbursement_id):
    ...
    for attachment in attachments:
        FileStorageService.DeleteImage(attachment.attachment)
    attachments.delete()
...
@login_required
@permission_required("payroll.delete_allowance")
def delete_payslip_allowance(request, payslip_id):
    """
    This method is used to delete allowances from a payslip and update the payslip data
    """
    ...
    if request.method == "POST":
        allowance_ids = request.POST.getlist('allowance_ids')
        if not allowance_ids:
            messages.warning(request, _("No allowances selected for deletion"))
            return redirect('view-created-payslip', payslip_id=payslip_id)
        with transaction.atomic():
            # Get the current pay_head_data
            pay_head_data = payslip.pay_head_data
            
            # Filter out the selected allowances
            deleted_allowances = [
                allowance for allowance in pay_head_data['allowances']
                if str(allowance['allowance_id']) in allowance_ids
            ]
            pay_head_data['allowances'] = [
                allowance for allowance in pay_head_data['allowances']
                if str(allowance['allowance_id']) not in allowance_ids
            ]

            # Recalculate gross pay and net pay
            total_allowances = sum(allowance['amount'] for allowance in pay_head_data['allowances'])
            deleted_allowances_total = sum(allowance['amount'] for allowance in deleted_allowances)
            pay_head_data['gross_pay'] = pay_head_data['basic_pay'] + total_allowances
            pay_head_data['taxable_gross_pay'] = pay_head_data['taxable_gross_pay'] - deleted_allowances_total
            pay_head_data['net_pay'] = pay_head_data['gross_pay'] - pay_head_data['total_deductions']
            # Update the payslip
            payslip.pay_head_data = pay_head_data
            payslip.save()
            Allowance.objects.filter(id__in=allowance_ids).delete()

            messages.success(request, _("Selected allowances have been removed from the payslip"))
            return HttpResponse('<script>window.location.reload();</script>')
...
@login_required
@permission_required("payroll.delete_deduction")
def delete_payslip_deduction(request, payslip_id):
    """
    This method is used to delete deductions from a payslip and update the payslip data
    """
    payslip = Payslip.objects.filter(id=payslip_id).first()
    if payslip is None:
        messages.error(request, _("Payslip not found"))
    ...
    if request.method == "POST":
        ...
        if not deduction_ids:
            messages.warning(request, _("No deductions selected for deletion"))
            return redirect('view-created-payslip', payslip_id=payslip_id)
        ...
        with transaction.atomic():
            ...
            # Get the appropriate deduction list based on type
            deduction_list_key = f"{deduction_type}_deductions"
            if deduction_type == "basic_pay":
                deduction_list_key = "basic_pay_deductions"
            elif deduction_type == "gross_pay":
                deduction_list_key = "gross_pay_deductions"
            elif deduction_type == "pretax":
                deduction_list_key = "pretax_deductions"
            elif deduction_type == "post_tax":
                deduction_list_key = "post_tax_deductions"
            elif deduction_type == "tax":
                deduction_list_key = "tax_deductions"
            elif deduction_type == "net":
                deduction_list_key = "net_deductions"

            # Filter out the selected deductions and calculate total amount to add back
            deleted_deductions = [
                deduction for deduction in pay_head_data[deduction_list_key]
                if str(deduction['deduction_id']) in deduction_ids
            ]
            pay_head_data[deduction_list_key] = [
                deduction for deduction in pay_head_data[deduction_list_key]
                if str(deduction['deduction_id']) not in deduction_ids
            ]

            # Calculate total amount to add back based on deduction type
            total_to_add_back = sum(deduction['amount'] for deduction in deleted_deductions)

            # Add back the amount to the corresponding pay type
            if deduction_type == "basic_pay":
                pay_head_data['basic_pay'] = pay_head_data.get('basic_pay', 0) + total_to_add_back
                pay_head_data['gross_pay'] = pay_head_data.get('gross_pay', 0) + total_to_add_back
                pay_head_data['taxable_gross_pay'] = pay_head_data.get('taxable_gross_pay', 0) + total_to_add_back
            elif deduction_type == "gross_pay":
                pay_head_data['gross_pay'] = pay_head_data.get('gross_pay', 0) + total_to_add_back
                pay_head_data['taxable_gross_pay'] = pay_head_data.get('taxable_gross_pay', 0) + total_to_add_back
            elif deduction_type == "pretax":
                ...
                pay_head_data['federal_tax'] = pay_head_data.get('federal_tax', 0) + total_to_add_back
            elif deduction_type == "net":
                pay_head_data['net_pay'] = pay_head_data.get('net_pay', 0) + total_to_add_back

            # Recalculate total deductions
            total_deductions = 0
            for key in ['basic_pay_deductions', 'gross_pay_deductions', 'pretax_deductions', 
                       'post_tax_deductions', 'tax_deductions', 'net_deductions']:
                if key in pay_head_data:
                    total_deductions += sum(deduction['amount'] for deduction in pay_head_data[key])

            # Reset payslip data to original values
            original_data = {
                "range": pay_head_data.get("range", ""),
                "net_pay": pay_head_data.get("gross_pay", 0) - total_deductions,
                "employee": pay_head_data.get("employee", ""),
                "end_date": pay_head_data.get("end_date", ""),
                "basic_pay": pay_head_data.get("basic_pay", 0),
                "gross_pay": pay_head_data.get("gross_pay", 0),
                "paid_days": pay_head_data.get("paid_days", 0),
                "allowances": pay_head_data.get("allowances", []),
                "start_date": pay_head_data.get("start_date", ""),
                "federal_tax": pay_head_data.get("federal_tax", 0),
                "loss_of_pay": pay_head_data.get("loss_of_pay", 0),
                "unpaid_days": pay_head_data.get("unpaid_days", 0),
                "contract_wage": pay_head_data.get("contract_wage", 0),
                "net_deductions": pay_head_data.get("net_deductions", []),
                "tax_deductions": pay_head_data.get("tax_deductions", []),
                "total_deductions": total_deductions,
                "pretax_deductions": pay_head_data.get("pretax_deductions", []),
                "taxable_gross_pay": pay_head_data.get("taxable_gross_pay", 0),
                "post_tax_deductions": pay_head_data.get("post_tax_deductions", []),
                "basic_pay_deductions": pay_head_data.get("basic_pay_deductions", []),
                "gross_pay_deductions": pay_head_data.get("gross_pay_deductions", [])
            }

            # Update the payslip data with original values
            payslip.pay_head_data = original_data
            payslip.save()

            # Delete the deduction records
            Deduction.objects.filter(id__in=deduction_ids).delete()

            messages.success(request, _("Selected deductions have been removed from the payslip"))
            return HttpResponse('<script>window.location.reload();</script>')
