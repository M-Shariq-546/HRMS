...
import shutil
from base.views import currency_symbol
...
logger = HorillaLogger("payroll.views")
...
def contract_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            contract = form.save()
            file_url = FileStorageService.UploadImage(contract.contract_document, image_type='contract_documents')
            contract.contract_document = file_url
            contract.save()
...
def contract_update(request, contract_id, **kwargs):
    ...
    if request.method == "POST":
        contract_document_change = request.FILES.getlist("contract_document")
        if contract_document_change:
            previous_contract_document = contract.contract_document
        ...
        if contract_form.is_valid():
            ...
            if contract_document_change:
                ...
                contract.contract_document = file_url
                contract.save()
                FileStorageService.DeleteImage(previous_contract_document)
...
def contract_delete(request, contract_id):
    ...
    try:
        contract = Contract.objects.get(id=contract_id)
        FileStorageService.DeleteImage(contract.contract_document)
        contract.delete()
...
def contract_filter(request):
    ...
    if field != "" and field is not None:
        ...
        # Sort each contract list by contract_start_date
        for entry in contracts:
            entry["list"].object_list = sorted(entry["list"].object_list, key=lambda c: c.contract_start_date)
...
def update_payslip_status(request, payslip_id):
    ...
    if payslip:
        ...
        if status == 'paid':
            ...
            if employee_id.get_mail() is None:
                messages.warning(request, _('Please set Employee Email Address first'))
                return HttpResponse("<script>window.location.reload()</script>")
            payslip.status = status
            payslip.save()
            if payslip.status == 'paid':
                from payroll.threadings.wise_thread import WiseThread
                from threading import Thread
                company_instance = Company.objects.get(id=payslip.employee_id.employee_work_info.company_id.id)
                thread = Thread(target=WiseThread, args=(request, payslip_id, employee_id, company_instance))
                thread.start()
            if not payslip.sent_to_employee:
    ...
    if view:
        ...
        query_filter = request.session.get('payslip_query_string', '')
        redirect_url = reverse("filter-payslip")
        if query_filter:
            return redirect(f"{redirect_url}?{query_filter}")
...
def update_payslip_status_no_id(request):
    ...
    if request.method == "POST":
        ...
        if status == 'paid':
            for slip in slips:
                if slip.sent_to_employee:
                    continue
                employee_id = slip.employee_id
                result_dict[employee_id]["employee_id"] = employee_id
                result_dict[employee_id]["instances"].append(slip)
                result_dict[employee_id]["count"] += 1
                if employee_id.get_mail() is None:
                    return JsonResponse({
                        "type": "warning",
                        "message": f"Please add Work Information (email) first to Employee: {str(slip.employee_id)} then retry"
                    })
...
def view_payslip_pdf(request, payslip_id):
    ...
    if Payslip.objects.filter(id=payslip_id).exists():
        ...
        if (
            ...
            data['salary_slip_date'] = end_date.strftime("%B %d, %Y")
            ...
            data["all_deductions"] = []
            data["federal_tax"] = [{"title": "Federal Tax", "amount": data.get("federal_tax", 0.0)}]
            ...
            return render(request, "payroll/payslip/salary-slip.html", context=data)
...
def view_created_payslip(request, payslip_id, **kwargs):
    ...
    wise_integration = CompanyIntegration.objects.get(company_id=company_instance, service_name='wise')
    payslip = Payslip.objects.filter(id=payslip_id).first()
    if payslip is None:
        messages.error(request, _("Payslip not found"))
        return redirect('view-payslip')
    leaves = LeaveRequest.objects.filter(employee_id=payslip.employee_id , status='approved' , start_date__gte=payslip.start_date ,end_date__lte=payslip.end_date)
    penalties = PenaltyAccounts.objects.filter(leave_request_id__in=leaves, leave_type_id__payment='unpaid', leave_request_id__status='approved')
    penalty_amount += sum(penalty.penalty_amount for penalty in penalties)
    if payslip is not None and (
        request.user.has_perm("payroll.view_payslip")
        or payslip.employee_id.employee_user_id == request.user
    ):
        # the data must be dictionary in the payslip model for the json field
        grouped_leaves = defaultdict(lambda: {"count": 0, "payment": None})
        for leave in leaves:
            leave_type_name = leave.leave_type_id.name
            grouped_leaves[leave_type_name]["count"] += 1
            grouped_leaves[leave_type_name]["payment"] = leave.leave_type_id.payment
        grouped_leaves = dict(grouped_leaves)
        data = payslip.pay_head_data
        data["employee"] = payslip.employee_id
        data['grouped_leaves'] = grouped_leaves
        ...
        data["json_data"]["payslip"] = payslip.id
        data["instance"] = payslip
        wise_dashboard_data = WiseService.GetWiseDashboardDataForSinglePayslip(wise_integration, payslip.employee_id.wise_recipient_id, payslip.id)
        data["wise_dashboard_data"] = wise_dashboard_data
        logger.info(f"========= wise dashboard data {wise_dashboard_data} =========")
...
def dashboard_department_chart(request):
    ...
    if is_ajax and request.method == "GET":
        ...
        for employee in employee_list:
            department.append(
                employee.employee_id.employee_work_info.department_id.department if employee.employee_id.employee_work_info.department_id is not None else ''
            )
...
def generate_payslip_pdf(template_path, context, html=False):
    ...
    try:
        ...
        path_wkhtmltopdf = shutil.which("wkhtmltopdf") or "/usr/local/bin/wkhtmltopdf"  # adjust as needed
        ...
        # Encode PDF to base64 for logging
        try:
            pdf_base64 = base64.b64encode(pdf).decode('utf-8')
        except Exception as encoding_error:
            logger.error(f"Error encoding PDF to base64: {encoding_error}")
...
def payslip_pdf(request, id):
    ...
    if Payslip.objects.filter(id=id).exists():
        ...
        if (
            ...
            data.update(
                {
                    "month_start_name": start_date.strftime("%B %d, %Y"),
                    "month_end_name": end_date.strftime("%B %d"),
                    'salary_slip_date': end_date.strftime("%B %d, %Y"),
                    ...
                    "currency": PayrollSettings.objects.first().currency_code,
                    "all_deductions": [],
            ...
            equalize_lists_length(data["allowances"], data["all_deductions"])
            data["zipped_data"] = zip(data["allowances"], data["all_deductions"])
            template_path = "payroll/payslip/salary-slip.html"
...
def get_wise_dashboard_data(request):
    company_id = request.session.get('selected_company')
    company_instance = Company.objects.get(id=company_id)
    wise_integration = CompanyIntegration.objects.get(company_id=company_instance, service_name='wise')
    data = WiseService.GetWiseDashboardData(wise_integration)
