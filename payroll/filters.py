...
logger = HorillaLogger('payroll_filters')
...
class PayslipFilter(FilterSet):
    ...
    employee_id = django_filters.ModelMultipleChoiceFilter(
        queryset=Employee.objects.none(),  # Empty by default
        widget=forms.SelectMultiple(),
    )
    start_date = django_filters.DateFilter(
        field_name="start_date",
        lookup_expr="gte",  # Use greater than or equal for range filtering
        widget=forms.DateInput(attrs={"type": "date"}),
    )
    ...
    def __init__(self, data=None, queryset=None, *, request=None, prefix=None):
        if data:
            ...
            for key in list(cleaned_data.keys()):
                if not cleaned_data[key] or cleaned_data[key][0] == "":
                    del cleaned_data[key]
        else:
            cleaned_data = data
        super().__init__(data=cleaned_data, queryset=queryset, request=request, prefix=prefix)
        active_contract_employees = Employee.objects.filter(contract_set__contract_status='active')
        if data and "employee_id" in data:
            selected_ids = data.getlist("employee_id")
            active_contract_employees = active_contract_employees.exclude(id__in=selected_ids)
        self.form.fields['employee_id'].queryset = active_contract_employees
        for field in self.form.fields.keys():
            self.form.fields[field].widget.attrs["id"] = f"{uuid.uuid4()}"
