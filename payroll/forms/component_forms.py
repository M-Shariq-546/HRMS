...
MAX_FILE_SIZE = int(os.environ.get("MAX_ATTACHMENT_SIZE"))
...
class AllowanceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["amount"].widget.attrs.update({"class": "oh-input"})
        self.fields["if_amount"].widget.attrs.update({"class": "oh-input"})
...
class DeductionForm(forms.ModelForm):
    ...
    def clean(self, *args, **kwargs):
        ...
        specific_employees = self.data.getlist("specific_employees")
        include_all = bool(self.data.get("include_active_employees"))
        condition_based = self.data.get("is_condition_based")
        ...
        if not specific_employees:
            cleaned_data["specific_employees"] = Employee.objects.filter(employee_work_info__company_id=self.request.session.get("selected_company"))
            print('all company employees ', cleaned_data['specific_employees'])
        else:
            if not condition_based and not specific_employees:
                raise forms.ValidationError({
                    "specific_employees": _("This field is required if not condition based or including all.")
                })
            cleaned_data["specific_employees"] = Employee.objects.filter(
                id__in=specific_employees
            )
        for field_name, field_instance in self.fields.items():
            if isinstance(field_instance, HorillaMultiSelectField):
                self.errors.pop(field_name, None)
    ...
    def save(self, commit: bool = ...) -> Any:
        ...
        specific_employees = self.cleaned_data.get('specific_employees')
        excluded_employees = self.cleaned_data.get('exclude_employees')
        excluded_employees = list(excluded_employees.values_list('id', flat=True))
        super().save(commit)
        if not specific_employees:
            specific_employees = Employee.objects.filter(employee_work_info__company_id=self.request.session.get("selected_company"))
            if excluded_employees:
                specific_employees = specific_employees.exclude(id__in=excluded_employees)
            if commit:
                self.instance.specific_employees.set(specific_employees)
...
class BonusForm(Form):
    ...
    company_id = forms.CharField(max_length=100, required=False, widget=forms.HiddenInput())
    ...
    is_multiple_allowance = forms.BooleanField(
        label="Recurring Allowance", initial=False, required=False, widget=forms.CheckboxInput()
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["is_multiple_allowance"].widget.attrs.update({"class": "oh-switch__checkbox","onchange": "toggleDateField(this)"})
    ...
    def clean(self):
        cleaned_data = super().clean()
        is_multiple_allowance = cleaned_data.get('is_multiple_allowance')
        date = cleaned_data.get('date')

        if not is_multiple_allowance and not date:
            raise forms.ValidationError({'date': 'Date is required when not a recurring allowance'})
...
class PayslipDeductionForm(ModelForm):
    ...
    verbose_name = _("Deduction")
    recurring_deduction = forms.BooleanField(
        label="Recurring Deduction",
        initial=False,
        required=False,
        widget=forms.CheckboxInput(attrs={"onchange": "toggleDateField(this)"})
    )
    ...
    class Meta:
        ...
        widgets = {
            "one_time_date": forms.DateInput(attrs={"type": "date", "required": False}),
            "update_compensation": forms.Select(attrs={"class": "oh-select oh-select-2"}),
        }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initial['one_time_date'] = datetime.date.today()
        for field_name, field in self.fields.items():
            if isinstance(field.widget, forms.CheckboxInput):
                field.widget.attrs.update({"class": "oh-switch__checkbox"})
            elif not isinstance(field.widget, forms.Select):  # Don't override Select widget classes
                field.widget.attrs.update({"class": "oh-input w-100"})
        self.fields['one_time_date'].required = False  # Also set required=False on the field itself

    def clean(self):
        cleaned_data = super().clean()
        is_multiple_deduction = cleaned_data.get('is_multiple_deduction')
        one_time_date = cleaned_data.get('one_time_date')

        if not is_multiple_deduction and not one_time_date:
            raise forms.ValidationError({'one_time_date': 'Date is required when not a recurring deduction'})
...
class ReimbursementForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["attachment"].widget.attrs.update({"class": "w-100"})
    ...
    def clean(self):
        ...
        # Validate file size for attachments (max 1MB per file)
        if "attachment" in self.files:
            max_size = MAX_FILE_SIZE  # 1MB in bytes
            for uploaded_file in self.files.getlist("attachment"):
                if uploaded_file.size > max_size:
                    self.add_error("attachment", forms.ValidationError(
                        f"File '{uploaded_file.name}' is too large. Maximum file size is 1MB. Current size: {uploaded_file.size / (1024*1024):.2f}MB"
                    ))
    ...
    def save(self, commit: bool = ...) -> Any:
        ...
        employee_id = self.data.get('employee_id') if self.data.get('employee_id') else self.initial['employee_id']
