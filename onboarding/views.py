...
def email_send(request):
    ...
    for cand_id in candidates:
        ...
        portal_url = f"{protocol}://{host}/onboarding/user-creation/{token}"
        ...
        # Send email using SynetechEmailService
        email_sent = email_service.send_email(
            emails=[candidate.email],
            subject=f"Hello {candidate.name}, Congratulations on your selection!",
            message=html_message,
            attachments=attachments,
            is_template=False  # Set to False since we're already rendering the template
        )
        ...
        try:
            # Check if CandidateStage already exists
            existing_stage = CandidateStage.objects.filter(candidate_id=candidate).first()
...
def employee_creation(request, token):
    ...
    if Employee.objects.filter(employee_user_id=user).first() is not None:
        ...
        if employee.employee_bank_details:
            ...
            return redirect("login")
        initial = Employee.objects.filter(email=user).first().__dict__
    form = EmployeeCreationForm(
        initial=initial
    )
    # form.errors.clear()
    if request.method == "POST":
        instance = Employee.objects.filter(email=user).first()
        form = EmployeeCreationForm(
            request.POST,
            instance=instance,
        )
