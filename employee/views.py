...
from .threads import EmployeeWelcomeMailsThread
...
logger = HorillaLogger('employee.views')
...
def employee_view_individual(request, obj_id, **kwargs):
    ...
    try:
        ...
        return HttpResponseRedirect(reverse("employee-view"))
    employee_leaves = (
    ...
    # Add activity_list for bonus_points.html include
    ...
    import operator
    try:
        points = BonusPoint.objects.get(employee_id=employee.id)
        ...
        # If payroll is installed, add requested bonus points
        if apps.is_installed("payroll"):
            ...
            requested_bonus_points = Reimbursement.objects.filter(
                employee_id=employee.id, type="bonus_encashment", status="requested"
            )
    ...
    logger.info(f'activity_list {activity_list}')
    return render(
        request,
...
def about_tab(request, obj_id, **kwargs):
    ...
    """
    employee = Employee.objects.get(id=obj_id)
    contracts = employee.contract_set.all().order_by('-contract_start_date') if apps.is_installed("payroll") else None
    employee_leaves = (
        employee.available_leave.all() if apps.is_installed("leave") else None
    )
    return render(
...
def document_create(request, emp_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            document = form.save()
            file_url = FileStorageService.UploadImage(document.document, image_type='documents')
...
def document_delete(request, id):
    ...
    try:
        ...
        if document:
            ...
            FileStorageService.DeleteImage(document_first.document)
            document.delete()
            messages.success(
                request,
                _(
...
def file_upload(request, id):
    ...
    if request.method == "POST":
        logger.info(f'inside the post method of document update view')
        previous_document = document_item.document
        form = DocumentUpdateForm(request.POST, request.FILES, instance=document_item)
        if form.is_valid():
            document = form.save()
            if previous_document:
                FileStorageService.DeleteImage(previous_document)
...
def employee_view(request):
    ...
    ).order_by(F("badge_id").asc(nulls_first=False))
...
def employee_view_new(request):
    ...
    """
    selected_company = request.session.get('selected_company')
    if selected_company is None or selected_company == 'all':
        messages.error(request, _("Please select a company first."))
        return redirect(employee_view)
    fields_check = DocumensoService.CheckSelectedFields(selected_company)
    if not fields_check:
        messages.error(request, _("Please map all fields and notification message before creating a new employee."))
        return redirect('integrations:integration_list')
    form = EmployeePersonalInfoForm()
...
def update_profile_image(request, obj_id):
    ...
    try:
        employee = Employee.objects.get(id=obj_id)
        previous_image = employee.employee_profile
        img = request.FILES["employee_profile"]
        employee.employee_profile = img
        employee.save()
        file_path = FileStorageService.UploadImage(employee.employee_profile, image_type="employee")
        employee.employee_profile = file_path
        employee.save()
        if previous_image:
            FileStorageService.DeleteImage(previous_image)
...
def employee_create_update_personal_info(request, obj_id=None):
    ...
    if form.is_valid():
        ...
        if obj_id is None:
            ...
            company_instance = Company.objects.get(id=selected_company)
            work_form.instance.company_id = company_instance
            work_form.save()
            ...
            try:
                thread = threading.Thread(
                    target=EmployeeWelcomeMailsThread,
                    args=(request, employee, company_instance),
                    daemon=True  
                )
                thread.start()
            except Exception as e:
            ...
            return redirect(
                f"employee-view-update/{employee.id}/",
...
def bonus_points_tab(request, emp_id):
    ...
    """
    employee_obj = Employee.objects.get(id=emp_id)
    points, created = BonusPoint.objects.get_or_create(employee_id=emp_id)
    if apps.is_installed("payroll"):
        Reimbursement = get_horilla_model_class(
            app_label="payroll", model="reimbursement"
        )
        ...
        if form.is_valid():
            ...
            if apps.is_installed("payroll"):
                ...
                Reimbursement.objects.create(
                    title=f"Bonus point Redeem for {employee}"[:35],
                ...
                messages.success(request, 'Bonus points redeemption requested successfully!')
            return HttpResponse("<script>window.location.reload();</script>")
    return render(
        request,
...
def employee_get_mail_log(request):
    ...
    for log in tracked_mails:
        try:
            log.to = ast.literal_eval(log.to) if isinstance(log.to, str) and log.to.startswith("[") else [log.to]
        except Exception:
            log.to = [log.to]
...
def employee_document_view(request):
    ...
    context = {}
    employee = Employee.objects.get(id=employee_id)
    documents_ids = employee.documenso_documents_list
    documents = DocumensoService.GetListDocuments(documents_ids,employee.employee_work_info.company_id)
    logger.info(f'======== company instance {employee.employee_work_info.company_id}    =====   documents_ids {documents_ids} ===== {documents}')
    context['data'] = documents.get('documents', [])
    context['documents'] = documents
    return render(request, 'tabs/employee_document_view.html', context)


def documenso_document_resend_url(request, document_id):
    logger.info('========== calling the view of resend documents ')
    company_id = request.session.get('selected_company')
    if company_id == 'all':
    ...
    resend_success = DocumensoService.ResendReminderDocuments(document_id, company_instance)
    if resend_success:
        messages.success(request, "Document Resent successfully")
    else:
        messages.error(request, "Error in sending Documents ")
