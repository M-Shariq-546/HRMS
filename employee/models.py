...
logger = HorillaLogger('employee.models')
...
class Employee(models.Model):
    ...
    employee_profile = models.ImageField(
        upload_to="employee/profile", 
        max_length=1000,
        null=True, 
        blank=True,
        help_text="This will be synced with FileStorage"
    )
    ...
    timezone = models.CharField(max_length=255, choices=[(tz, tz) for tz in all_timezones], default="UTC")
    is_from_onboarding = models.BooleanField(
        default=False, null=True, blank=True, editable=False
    )
    send_welcome_email_to_company = models.BooleanField(
        default=True, null=True, blank=True,verbose_name=_("Send welcome email to entire company")
    )
    send_welcome_email_to_employee = models.BooleanField(
        default=True, null=True, blank=True, verbose_name=_("Send welcome email to employee")
    )
    send_onboarding_email_to_employee = models.BooleanField(
        default=True, null=True, blank=True, verbose_name=_("Send onboarding email to employee")
    )
    documenso_documents_list = models.CharField(max_length=255 , null=True , blank=True)
    ...
    def get_mail(self):
        """
        This method is used to return the primary email of the employee
        """
        return self.email
    ...
    def get_avatar(self):
        """
        Method will return the avatar or path to the profile image
        """
        # Default avatar URL
        url = f"https://ui-avatars.com/api/?name={self.get_full_name()}&background=random"
        
        if self.employee_profile:
            return FileStorageService.GetImageUrl(self.employee_profile)
        return url
    ...
    def get_working_duration(self) -> str:
        if self.employee_work_info.date_joining:
            date_joining = self.employee_work_info.date_joining
            today = date.today()
            
            # Calculate the difference
            diff = relativedelta(today, date_joining)
    ...
    def delete(self, *args, **kwargs):
        """
        Override delete method to clean up FileStorage
        """
        super().delete(*args, **kwargs)
        if self.employee_profile:
            FileStorageService.DeleteImage(self.employee_profile)
...
class PolicyMultipleFile(HorillaModel):
    """
    Model for storing multiple file attachments for policies.
    Handles both local storage and FileStorage bucket storage.
    """

    attachment = models.FileField(
        upload_to="employee/policies",
        null=True,
        blank=True,
        verbose_name=_("Policy Attachment")
    )

    def get_file_url(self) -> str:
        """
        Returns the URL for the attachment based on its storage location.
        
        Returns:
            str: The URL of the attachment file
        """
        file_service = FileStorageService()
        return file_service.GetImageUrl(self.attachment)
     
    def __str__(self) -> str:
        return f"Policy Attachment: {self.attachment}"


class Policy(HorillaModel):
    """
    Model for storing company policies with multiple file attachments.
    """
    ...
    def get_attachment_urls(self) -> list[str]:
        """
        Returns a list of URLs for all attachments associated with this policy.
        Handles both FileStorage and local storage attachments.
        """
        return [attachment.get_file_url() for attachment in self.attachments.all()]

    def delete(self, *args, **kwargs):
        """
        Override delete method to clean up attachments
        """
        super().delete(*args, **kwargs)
...
class DisciplinaryAction(HorillaModel):
    ...
    attachment = models.FileField(
        upload_to="employee/discipline",
        max_length=1000,  # To store FileStorage URLs
        null=True,
        blank=True,
    )
    ...
    def save(self, *args, **kwargs):
        is_new = self.pk is None
        if not is_new:
            old_instance = DisciplinaryAction.objects.get(pk=self.pk)
            old_document = old_instance.attachment
    ...
    def get_attachment_url(self):
        ...
        file_service = FileStorageService()
        file_url = file_service.GetImageUrl(self.attachment)
        return file_url
