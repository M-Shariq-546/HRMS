...
logger = HorillaLogger('employee.policies')
...
def create_policy(request):
    ...
    if request.method == "POST":
        company = request.POST.get("company_id") if request.POST.get("company_id") else instance.company_id.all()
        form = PolicyForm(request.POST, request.FILES, instance=instance)
        if form.is_valid():
            instance, attachments = form.save()
            if instance.attachments:
                for attachment in instance.attachments.all():
                    file_url = FileStorageService.UploadImage(attachment.attachment, image_type='employee')
...
def delete_policies(request):
    ...
    try:
        ...
        policies = Policy.objects.filter(id__in=ids)
        for policy in policies:
            for file in policy.attachments.all():
                file_service = FileStorageService()
                file_service.DeleteImage(file.attachment)
        count, dict = policies.delete()
        if count == 0:
...
def remove_attachment(request):
    ...
    policy_id = request.GET["policy_id"]
    policy = Policy.objects.get(id=policy_id)
    attachments = PolicyMultipleFile.objects.filter(id__in=ids)
...
def create_actions(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                # First save the form without committing to get the instance
                disciplinary_action = form.save()
                # Handle file upload if present
                file_url = FileStorageService.UploadImage(disciplinary_action.attachment, image_type='employee')
                disciplinary_action.attachment = file_url
                # Get employee IDs for notification
                employee_ids = form.cleaned_data["employee_id"]
                for employee in employee_ids:
                    user = employee.employee_user_id
                    employees.append(user)

                # Save the instance and many-to-many relationships
                ...
                messages.error(request, _("Failed to save disciplinary action. Please try again."))
                return HttpResponse("<script>window.location.reload()</script>")
...
def update_actions(request, action_id):
    ...
    employees = []
    if request.method == "POST":
        previous_document = action.attachment
        ...
        if form.is_valid():
            ...
            if previous_document:
                logger.info(f'Deleting previous disciplinary action attachment =============== {previous_document}')
    ...
    for dis_emp in dis.employee_id.all():
        ...
        if action_type == "dismissal" or action_type == "suspension":
            ...
            if user.is_active:
                ...
                user.is_active = True
                user.save()
    FileStorageService.DeleteImage(dis.attachment)
    dis.delete()
