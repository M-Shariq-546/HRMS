...
class Candidate(HorillaModel):
    ...
    def get_avatar(self):
        ...
        if self.profile:
            url = FileStorageService.GetImageUrl(self.profile)
            return url
        else:
            return url
    ...
    def get_resume(self):
        """
        Method will return the resume url
        """
        if self.resume:
            url = FileStorageService.GetImageUrl(self.resume)
            return url
        return None
...
def send_email_to_candidate(sender, instance, **kwargs):
    host = getattr(instance, "host", None)
    protocol = getattr(instance, "protocol", None)
    user = getattr(instance, "_request_user", None)
    is_edited = getattr(instance, "is_edited", None)

    if user:
        Thread(target=send_email_to_candidate_thread, args=(instance, user, is_edited, host, protocol)).start()
