...
from employee.validations import *
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if candidate.profile:
                file_url = FileStorageService.UploadImage(candidate.profile, image_type='candidate')
                candidate.profile = file_url
                candidate.save()
            if candidate.resume:
                file_url = FileStorageService.UploadImage(candidate.resume, image_type='candidate')
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if candidate_obj.profile:
                    file_url = FileStorageService.UploadImage(candidate_obj.profile, image_type='candidate')
                    candidate_obj.profile = file_url
                    candidate_obj.save()
                if candidate_obj.resume:
                    file_url = FileStorageService.UploadImage(candidate_obj.resume, image_type='candidate')
                    candidate_obj.resume = file_url
                    candidate_obj.save()
                candidate_obj.save()
...
def interview_schedule(request, cand_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            interview = form.save()
            interview._request_user = request.user
            interview.is_edited = False
            interview.host = request.get_host()
            interview.protocol = request.scheme
            interview.save()
...
def send_acknowledgement(request):
    ...
    for candidate in candidates:
        ...
        status, message = required_fields_check_candidate(request, candidate)
        ...
        context = template.Context(
            {
                ...
                "Sender_Company": request.user.employee_get.employee_work_info.company_id if request.user.employee_get.employee_work_info.company_id else "Our Company",
                "Sender_Job_position": request.user.employee_get.employee_work_info.job_position_id if request.user.employee_get.employee_work_info.job_position_id else "Admin Of Horilla",
                "Sender_Email": request.user.employee_get.get_email() if request.user.employee_get.get_email() else "info@horilla.synetechq.com",
                "Candidate_Full_name": candidate.get_full_name() if candidate.get_full_name() else "",
                "Candidate_Company": candidate.get_company if candidate.get_company else "",
                "Candidate_Job_position": candidate.get_job_position if candidate.get_job_position else "",
                "Candidate_Email": candidate.get_email if candidate.get_email else "",
            }
        )

        render_bdy = template_bdy.render(context)
        logger.info(f"send_to_mail inside send_acknowledgement functions of service : {candidate.email}")
        ...
        logger.info(f"success inside send_acknowledgement functions of service : {success}")
